...
QBCore.Commands.Add('911p', 'Police Report', {{name='message', help='Message to be sent'}}, false, function(source, args)
    ...
    TriggerEvent("wf-alerts:svNotify911", message, name, coords)
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'police' and v.PlayerData.job.onduty then
...
RegisterNetEvent('police:server:policeAlert', function(text)
    ...
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    print(src, coords.x, coords.y)
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'police' and v.PlayerData.job.onduty then
            local alertData = {title = 'New Call', coords = coords --[[{coords.x, coords.y, coords.z}]], description = text}
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
